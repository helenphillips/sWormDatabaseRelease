...
table(authors$I.consent.to.my.data.being.made.open.access)
# There is a negative

table(authors$I.would.like.to.be.an.author.on.this.data.paper)
## Two no's

authors <- droplevels(authors[authors$I.would.like.to.be.an.author.on.this.data.paper == "Yes",])
# 124
...
"If.you.feel.comfortable.doing.so..please.indicate.why.you.no.longer.want.us.to.share.the.data", 
"Please.can.you.give.some.details.about.your.data..e.g...study.location..sample.dates..to.ensure.we.are.referring.to.the.same.data.")
...
authors$middleInits[authors$middleInits == "Viktorovna."] <- "V."
authors$middleInits[authors$middleInits == "Rahman."] <- "R."

# Where people two initials with no space
authors$middleInits <- gsub("(\\S)(\\S)(\\.)", "\\1\\.\\ \\2\\3" , authors$middleInits)

authors$middleInits[authors$middleInits == "L C."] <- "L. C."


authors$FullName <- paste(authors$firstName, authors$middleInits, authors$lastName)
authors$FullName <- gsub("  ", " ", authors$FullName)

# There's some authors who signed up but didn't need to
# 
authors <- authors[-(grep("Gongalsky", authors$lastName)), ]
authors <- authors[-(grep("Eisenhauer", authors$lastName)), ]
authors <- authors[-(grep("Cameron", authors$lastName)), ]
...
# And there are people who signed up twice
authors[duplicated(authors$FullName),'FullName']
# Just checking

# authors <- authors[-(duplicated(authors$FullName)),]

authors$FullName[order(authors$lastName)]

authors <- authors[-(grep("Darío J. Díaz Cosón", authors$FullName)),]

authors <- authors[-(grep("Mónica Gutiérrez López", authors$FullName)[2]),]
authors <- authors[-(grep("Victoria Nuzzo", authors$FullName)[2]),]
authors <- authors[-(grep("Sandy M. Smith", authors$FullName)[2]),]

## 

## Someone got their country wrong

authors$Country[authors$Country == "Göttingen"] <- "Germany"
...
authors$City[authors$City == "Ithaca"] <- "Ithaca, NY"
authors$City[authors$City == "Jyvaskyla"] <- "Jyväskylä"
...
authors$City <- as.factor(authors$City)
