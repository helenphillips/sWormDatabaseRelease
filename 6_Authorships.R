...
## unique sites
length(unique(sites$ID))
## 8338
length(unique(sites$Study_site))
## 8335
...
files_sites <- unique(sites$file)
files_bib <- unique(bib$file)

## Sanity check
files_bib %in% files_sites #there are two files not in sites
files_sites %in% files_bib


# bib <- droplevels(bib[bib$file %in% files,])

length(unique(sites$file)) == nrow(bib)
## 205
...
## I left the sWorm people in, as we do need their permission